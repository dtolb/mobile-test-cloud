...
describe('lib.s3', function () {
	...
	describe('::uploadResults', function () {
		...
		beforeEach(function () {
			...
			s3FileLoaction = sprintf('/repo/%s/branch/%s/pr-number/%s/%s.json',
						testInfo.github.repo,
						testInfo.github.branch,
						testInfo.github.PRnumber,
						testInfo.github.sha);
			s3Url = sprintf('https://%s.s3.amazonaws.com',config.s3.bucket);
		});
		describe('When s3 upload is successful', function () {
			var s3Nock;
			beforeEach(function () {
				s3Nock = nock(s3Url)
					.put(s3FileLoaction, JSON.stringify(testInfo.results))
					.reply(200);
			});
			it('should attach the location to testInfo.s3.location', function () {
				return s3.uploadResults(testInfo)
					.then(function (res) {
						expect(res.s3.location).to.equal(s3Url + s3FileLoaction);
					});
			});
